...
void print_free_list() {
    list_entry_t* list_elem = &free_list;
    cprintf("### print_free_list start ###\n");
    while ((list_elem = list_next(list_elem)) != &free_list) {
        struct Page* page = le2page(list_elem, page_link);
        if (page->property > 0) {
            cprintf("Page Address=0x%x    Property=%d\n", page, page->property);
        }
    }
    cprintf("### print_free_list end ###\n");
}
