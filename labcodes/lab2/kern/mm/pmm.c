...
page_remove_pte(pde_t *pgdir, uintptr_t la, pte_t *ptep) {
    ...
    /*
    pde_t dictionary_entry = pgdir[PDX(la)];
    if ((dictionary_entry & PTE_P) == 0) return;
    
    pte_t* page_table = KADDR(PTE_ADDR(dictionary_entry));
    pte_t page_table_entry = page_table[PTX(la)];
    if ((page_table_entry & PTE_P) == 0) return;     
    */
